...
def rasch_model(data, max_students=None):
    """
    2PL (Bir parametrli emas, ikki parametrli) IRT modeli: p_ij = sigmoid(a_j*(theta_i - beta_j))
    MLE (L2 prior bilan) orqali theta (qobiliyat), beta (qiyinlik) va a (diskriminatsiya) ni baholaydi.
    ...
    # Boshlang'ich baholar (logit prop) va diskriminatsiya (1.0)
    ...
    # 2PL MLE (L2 prior: theta~N(0,1/λ), beta~N(0,1/λ), a~N(1,1/λ_a))
    REG_THETA = REG_LAMBDA
    REG_BETA = REG_LAMBDA
    REG_A = 0.05
    ...
    for _ in range(max_iter):
        ...
        # Ehtimolliklar
        logits = a[np.newaxis, :] * (theta[:, np.newaxis] - beta[np.newaxis, :])
        np.clip(logits, -15, 15, out=logits)
        p = expit(logits)
        residuals = data - p
        
        # Theta yangilanishi
        # gradient_i = sum_j a_j * (x_ij - p_ij) - λ*theta_i
        # hessian_i = sum_j a_j^2 * p(1-p) + λ
        grad_theta = np.sum(a[np.newaxis, :] * residuals, axis=1) - REG_THETA * theta
        hess_theta = np.sum((a[np.newaxis, :] ** 2) * p * (1 - p), axis=1) + REG_THETA
        update_theta = np.where(hess_theta > 1e-10, grad_theta / hess_theta, 0.0)
        theta += update_theta
        
        # Beta yangilanishi
        # gradient_j = -sum_i a_j * (x_ij - p_ij) - λ*beta_j
        # hessian_j = sum_i a_j^2 * p(1-p) + λ
        grad_beta = -np.sum(a[np.newaxis, :] * residuals, axis=0) - REG_BETA * beta
        hess_beta = np.sum((a[np.newaxis, :] ** 2) * p * (1 - p), axis=0) + REG_BETA
        update_beta = np.where(hess_beta > 1e-10, grad_beta / hess_beta, 0.0)
        beta += update_beta
        
        # a yangilanishi
        # gradient_a_j = sum_i (x_ij - p_ij) * (theta_i - beta_j) - REG_A*(a_j - 1)
        # hessian_a_j ≈ sum_i p(1-p) * (theta_i - beta_j)^2 + REG_A
        diff = (theta[:, np.newaxis] - beta[np.newaxis, :])
        grad_a = np.sum(residuals * diff, axis=0) - REG_A * (a - 1.0)
        hess_a = np.sum(p * (1 - p) * (diff ** 2), axis=0) + REG_A
        update_a = np.where(hess_a > 1e-10, grad_a / hess_a, 0.0)
        a += update_a
        # Chegaralash (diskriminatsiya > 0)
        np.clip(a, 0.3, 3.0, out=a)
        
        # Konvergensiya
        if max(np.max(np.abs(update_theta)), np.max(np.abs(update_beta)), np.max(np.abs(update_a))) < tol:
            break
    ...
    for i in range(n_students):
        # Har bir talaba uchun alohida MLE
        student_responses = data[i]
        raw_score = student_scores[i]
        
        # Boshlang'ich baho - har bir talaba uchun alohida
        prop = (raw_score + 0.5) / (n_items + 1)
        prop = np.clip(prop, 1e-6, 1 - 1e-6)
        theta_i = np.log(prop / (1 - prop))
        
        # MLE iteratsiyalari (stabilizatsiya uchun L2 prior bilan)
        for iteration in range(100):
            logits = theta_i - beta
            logits = np.clip(logits, -15, 15)
            p = expit(logits)
            
            gradient = np.sum(student_responses - p) - REG_LAMBDA * theta_i
            hessian = np.sum(p * (1 - p)) + REG_LAMBDA
    ...
    # Abilities ni markazlashtirish (mean = 0)
    theta = theta - np.mean(theta)
